...
import com.hashnightflux.vag.annotation.Validator;
import com.hashnightflux.vag.client.util.Easings;
                ...
                resources.forEach(resourceLocation ->
                {
                    ...
                    manager.getResource(resourceLocation).ifPresent(resource ->
                    {
                        try(Reader reader = new BufferedReader(new InputStreamReader(resource.open(), StandardCharsets.UTF_8)))
                        {
                            ...
                            {
                                VanillaAnimatedGuns.LOGGER.error("Couldn't load data file {} as it is missing or malformed. Using default gun data", resourceLocation);
                                map.putIfAbsent((GunItem) item, new Gun());
                            }
                        }
                        catch(InvalidObjectException e)
                        {
                            VanillaAnimatedGuns.LOGGER.error("Missing required properties for {}", resourceLocation);
                            e.printStackTrace();
                        }
                        catch(IOException e)
                        {
                            VanillaAnimatedGuns.LOGGER.error("Couldn't parse data file {}", resourceLocation);
                        }
