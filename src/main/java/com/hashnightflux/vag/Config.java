...
import com.hashnightflux.vag.client.render.crosshair.Crosshair;
...
public class Config
{
    ...
    public static class Common
    {
        ...
        public final Grenades dynamites;
        ...
        public Common(ForgeConfigSpec.Builder builder)
        {
            builder.push("common");
            {
                ...
                this.dynamites = new Grenades(builder);
    ...
    public static class AggroMobs
    {
        ...
        public final ForgeConfigSpec.DoubleValue soundRange;
        ...
        public AggroMobs(ForgeConfigSpec.Builder builder)
        {
            builder.comment("Properties relating to mob aggression").push("aggro_mobs");
            {
                ...
                this.soundRange = builder.comment("Any mobs within a sphere of this radius will aggro when hearing a shot.").defineInRange("soundRange", 20.0, 0.0, Double.MAX_VALUE);
    ...
    public static class Grenades
    {
        ...
        {
            builder.comment("Properties relating to dynamites").push("dynamites");
            {
    ...
    public static class EffectCriteria
    {
        ...
        public EffectCriteria(ForgeConfigSpec.Builder builder, double radius, int durationMax, int durationMin, double angleEffect, double angleAttenuationMax, boolean raytraceOpaqueBlocks)
        {
            builder.push("effect_criteria");
            {
                ...
                this.durationMax = builder.comment("Effect will have this duration (in ticks) if the dynamite is directly at the player's eyes while looking directly at it.").defineInRange("durationMax", durationMax, 0, Integer.MAX_VALUE);
                this.durationMin = builder.comment("Effect will have this duration (in ticks) if the dynamite is the maximum distance from the player's eyes while looking directly at it.").defineInRange("durationMin", durationMin, 0, Integer.MAX_VALUE);
                this.angleEffect = builder.comment("Angle between the eye/looking direction and the eye/dynamite direction must be no more than half this many degrees to have an effect.").defineInRange("angleEffect", angleEffect, 0, 360);
                this.angleAttenuationMax = builder.comment("After duration is attenuated by distance, it will be further attenuated depending on the angle (in degrees) between the eye/looking direction and the eye/dynamite direction. This is done by multiplying it by 1 (no attenuation) if the angle is 0; and by this value if the angle is the maximum within the angle of effect.").defineInRange("angleAttenuationMax", angleAttenuationMax, 0.0, 1.0);
                this.raytraceOpaqueBlocks = builder.comment("If true, the effect is only applied if the line between the eyes and the dynamite does not intersect any non-liquid blocks with an opacity greater than 0.").define("raytraceOpaqueBlocks", raytraceOpaqueBlocks);
            }
            builder.pop();
        }
    }
